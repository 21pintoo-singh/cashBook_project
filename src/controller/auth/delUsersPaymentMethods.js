...
const deletePayment = async (req, res) => {
    try{
        ...
        if(getUser.paymentMethods.indexOf(delpaymentMethods)==-1) return res.status(404).send({status:false,message:"paymentMethods not found"})
        getUser.paymentMethods.splice(getUser.paymentMethods.indexOf(delpaymentMethods),1)
        await getUser.save();

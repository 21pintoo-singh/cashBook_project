...
const deleteCategory = async (req, res) => {
    try{
        ...
        if(getUser.category.indexOf(delCategory)==-1) return res.status(404).send({status:false,message:"Category not found"})
        //let arr=[]
        getUser.category.splice(getUser.category.indexOf(delCategory),1)
        await getUser.save();
